...
QPair<int, int>ScintillaEdit::find_text(int flags, const char *text, int cpMin, int cpMax) const {
    Sci_TextToFindFull ft = {{0, 0}, 0, {0, 0}};
    ft.chrg.cpMin = cpMin;
    ft.chrg.cpMax = cpMax;
    ...
    int start = static_cast<int>(send(SCI_FINDTEXTFULL, flags, (uptr_t) (&ft)));

    return QPair<int,int>(start, static_cast<int>(ft.chrgText.cpMax));
}
...
QByteArray ScintillaEdit::get_text_range(int start, int end) const {
    ...
    QByteArray ba(length+1, '\0');
    Sci_TextRangeFull tr = {{start, end}, ba.data()};
